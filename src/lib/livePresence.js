...
class LivePresence extends EventEmitter {
  constructor() {
    ...
    // Counter of conversions seen since process start (for "today" stat)
    this.conversionsThisProcess = 0;
   ...
   */
  converted({ click_id, term, text, href }) {
    if (!click_id) return;
    this.conversionsThisProcess += 1;
    const v = this.visitors.get(click_id);
    if (!v) return;       // visitor not currently tracked, just bump global counter
    v.converted = true;
